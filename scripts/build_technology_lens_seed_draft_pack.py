#!/usr/bin/env python3
"""Build a bulk seed draft pack for uncovered technology lens domains."""
...
GROUP_STATUS = "auto_seed_curated"
CONTENT_STATUS = "auto_seed_curated"
SOURCE_BASIS_TYPE = "technology_lens_seed_batch"
GENERATED_BY = "build_technology_lens_seed_draft_pack.py"
PACK_ID = "technology-lens-seed-draft-pack-v1"
...
def build_taxonomy_terms(group: dict[str, object]) -> list[tuple[str, dict[str, object], bool]]:
    taxonomy = group.get("taxonomy") or {}
    terms: list[tuple[str, dict[str, object], bool]] = []

    primary_domain = taxonomy.get("primary_tech_domain")
    if primary_domain:
        terms.append(("tech_domain", primary_domain, True))
    for term in taxonomy.get("secondary_tech_domains", []):
        terms.append(("tech_domain", term, False))

    primary_subdomain = taxonomy.get("primary_tech_subdomain")
    if primary_subdomain:
        terms.append(("tech_subdomain", primary_subdomain, True))
    for term in taxonomy.get("secondary_tech_subdomains", []):
        terms.append(("tech_subdomain", term, False))

    for term in taxonomy.get("strategies", []):
        terms.append(("strategy", term, bool(term.get("is_primary"))))
...
def build_pack_json(selected_groups: list[dict[str, object]]) -> dict[str, object]:
    ...
    return {
        "sample_scope": {
            "pack_id": PACK_ID,
            "generated_from": "work/05_dashboard/data-contracts/technology-lens.json + technology-lens-curation-queue.csv",
            "purpose": "Bulk seed draft pack for technology domains that do not yet have curated groups.",
...
def main() -> None:
    ...
    parser.add_argument("--groups-per-domain", type=int, default=1)
    ...
    for row in queue_rows:
        if row.get("domain_priority_tier") != "seed_curated":
            continue
    ...
    for domain_rank, queue_row in enumerate(selected_queue_rows, start=1):
        tech_domain_id = queue_row["tech_domain_id"]
        group = groups_by_key[(tech_domain_id, queue_row["policy_item_group_id"])]
        group_id = f"PIG-SEED-{tech_domain_id}-{domain_rank:02d}"
        group_payload = {
        ...
        group_rows.append(
            {
                ...
                "display_order": domain_rank,
                "notes": f"seed draft from {group['policy_item_group_id']} / {group.get('source_policy_item_id', '')}",
            }
        )
        display_rows.append(
            {
                "display_text_id": f"DSP-GRP-SEED-{tech_domain_id}-{domain_rank:02d}",
                ...
                "source_basis_type": SOURCE_BASIS_TYPE,
                "notes": f"seed draft from {group['policy_item_group_id']}",
            }
        )
        ...
        for member_index, member in enumerate(group.get("member_items", []), start=1):
            member_rows.append(
                {
                    "policy_item_group_member_id": f"PGM-SEED-{tech_domain_id}-{domain_rank:02d}-{member_index:02d}",
                    "policy_item_group_id": group_id,
        ...
        for taxonomy_index, (taxonomy_type, term, is_primary) in enumerate(build_taxonomy_terms(group), start=1):
            taxonomy_rows.append(
                {
                    "policy_item_group_taxonomy_map_id": f"PGTM-SEED-{tech_domain_id}-{domain_rank:02d}-{taxonomy_index:02d}",
                    "policy_item_group_id": group_id,
                    ...
                    "review_status": "reviewed",
                    "notes": f"seed draft from {group['policy_item_group_id']} ({term.get('review_status', '')})",
                }
            )
        ...
        for content_index, content in enumerate(group.get("contents", []), start=1):
            content_id = f"PIC-SEED-{tech_domain_id}-{domain_rank:02d}-{content_index:02d}"
            content_rows.append(
                {
                    ...
                    "notes": f"seed draft from {content['policy_item_content_id']}",
                }
            )
            display_rows.append(
                {
                    "display_text_id": f"DSP-CNT-SEED-{tech_domain_id}-{domain_rank:02d}-{content_index:02d}",
            ...
            for evidence_index, evidence in enumerate(content.get("evidence", []), start=1):
                evidence_rows.append(
                    {
                        "policy_item_content_evidence_link_id": f"PCEL-SEED-{tech_domain_id}-{domain_rank:02d}-{content_index:02d}-{evidence_index:02d}",
                        "policy_item_content_id": content_id,
    ...
    write_csv(
        out_dir / "seed_domain_selection.csv",
        selection_rows,
        [
            "seed_rank",
            "tech_domain_id",
    ...
    write_json(out_dir / "technology-lens-seed-draft-pack.json", pack_json)
    ...
    summary = {
        ...
        "selected_domains": selection_rows,
        "out_dir": str(out_dir),
    }
    write_json(Path(args.out_summary_json), summary)
    print(f"Technology lens seed draft groups: {len(group_rows)}")
